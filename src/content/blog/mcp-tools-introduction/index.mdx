...
```toml title="pyproject.toml" showLineNumbers=false
[project]
name = "kimai-mcp"
version = "0.1.0"
...
```json "end" "begin" "id" "description" "activity" ""
[
  {
    "activity": 0,
    "project": 0,
...
```json collapse={8-16,19-44,47-63}
  {
    "tags": [],
    "id": 435,
...
```python title="kimaiapi/client.py" startLineNumber={10}
...
```python title="main.py" add={1-3,6-10,14-18,20-24}
...
<Callout type="note">
  ...
  <Callout variant="explanation" defaultOpen={false}>
    ```python title="main.py" add={5-6,13-24} del={7,25} startLineNumber={14}
    @mcp.tool()
    async def get_projects() -> Json:
      """Returns a list of projects on Kimai."""
...
```python title="kimaiapi/client.py" startLineNumber={45}
...
```python title="main.py" startLineNumber={38}
@mcp.tool()
...
```python title="main.py" add={1, 5-8}
...
```python title="main.py" startLineNumber={89}
@mcp.tool()
...
we will cover in the next posts